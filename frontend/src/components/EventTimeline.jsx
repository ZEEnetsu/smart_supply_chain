...
const typeConfig = {
  DELAY:     { color: 'text-red-400',   bg: 'bg-red-100',    label: 'Delay'     },
  WEATHER:   { color: 'text-amber-400', bg: 'bg-amber-100',  label: 'Weather'   },
  BREAKDOWN: { color: 'text-red-400',   bg: 'bg-red-100',    label: 'Breakdown' },
  CUSTOMS:   { color: 'text-purple-400',bg: 'bg-purple-100', label: 'Customs'   },
  DEPARTED:  { color: 'text-blue-400',  bg: 'bg-blue-100',   label: 'Departed'  },
  ARRIVED:   { color: 'text-green-400', bg: 'bg-green-100',  label: 'Arrived'   },
};
...
export default function EventTimeline() {
  ...
  const { data, isLoading } = useGetShipmentTimelineQuery(shipmentId, {
    ...
    pollingInterval:10000, //10s  [1s = 1000ms] 
              ...
              return (
                <div key={event._id} className="flex gap-3 relative">
                  ...
                  <div className={`
                    ...
                    shrink-0 z-10 border-2 bg-zinc-800 border-zinc-700
                  `}>
                  ...
                  <div className="flex-1 pb-1">
                    ...
                    <div className="flex gap-2 mt-1 items-center">
                      <span className={`
                        text-xs px-1.5 py-0.5 rounded text-gray-500
                      `}>
                        Risk : <span className={`${cfg.color}`}>{event.riskLevel}</span>
                      </span>
                      {event.cause !== 'none' && (
